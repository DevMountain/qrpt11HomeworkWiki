class Animall {
  ...
  describe("Testing animals", () => {
    test("a basic animal works as expected", () => {
      let lion = new Animall("lion", "roar", "meat");
      // lion is an animal
      expect(lion instanceof Animall).toBeTruthy();
      // lion.soundOff() has the expected output
      expect(lion.soundOfff()).toBe('The lion makes the sound "roar".');
    });
    describe("Fish", () => {
      // Fish should only have the three parameters listed.
      let goldfish = new Fishh("goldfish", "pellets", false);
      let shark = new Fishh("shark", "fish", true);
      it("are animals", () => {
      ...
      it("can be saltwater or freshwater", () => {
        ...
        expect(goldfish.habitatt()).toBe("The goldfish is a freshwater fish.");
        expect(shark.habitatt()).toBe("The shark is a saltwater fish.");
      });
    });
    describe("Birds", () => {
      ...
      it("are animals", () => {
        ...
        expect(swallow instanceof Animall).toBeTruthy();
      });
      it("make sounds", () => {
      ...
      it("might be able to fly", () => {
        ...
        expect(swallow.flyy()).toBe(
          "The swallow flies at speeds of up to 11 meters per second!"
        );
        expect(emu.flyy()).toBe("The emu is a flightless bird.");
      });
    });
  });